...
from utils.runtime_ctx.context import new_context, Context
...
from utils.error.classifier import ErrorClassifier, classify_error
...
class GraphService:
    ...
    # 流式运行（原始迭代器）：本地调用使用
    def stream(
        self, payload: Dict[str, Any], run_config: RunnableConfig, ctx=Context
    ) -> Iterable[Any]:
        ...
        t0 = time.time()
        try:
            items = self._get_graph(ctx).stream(
                stream_input, stream_mode="messages", config=run_config, context=ctx
            )
    ...
    async def run(self, payload: Dict[str, Any], ctx=None) -> Dict[str, Any]:
        ...
        try:
            ...
            # 使用错误分类器分类错误
            err = self.error_classifier.classify(
                e, {"node_name": "run", "run_id": run_id}
            )
            # 记录详细的错误信息和堆栈跟踪
            logger.error(
    ...
    async def stream_sse(
        self, payload: Dict[str, Any], ctx=None
        ...
        try:
            async for chunk in self.astream(
                payload, graph, run_config=run_config, ctx=ctx
            ):
    ...
    async def run_node(self, node_id: str, payload: Dict[str, Any], ctx=None) -> Any:
        ...
        node_func, input_cls, output_cls = graph_helper.get_graph_node_func_with_inout(
            self.graph.get_graph(), node_id
        )
    ...
    def graph_inout_schema(self) -> Any:
        ...
        return {
            "input_schema": _graph_input.model_json_schema(),
            "output_schema": _graph_output.model_json_schema(),
        }
    ...
    async def astream(
        ...
        def producer():
            ...
            try:
                ...
                if cancelled.is_set():
                    logger.info(
                        f"Producer cancelled before start for run_id: {ctx.run_id}"
                    )
                    return
                ...
                for sm in server_msgs_iter:
                    ...
                    if cancelled.is_set():
                        logger.info(
                            f"Producer cancelled during iteration for run_id: {ctx.run_id}"
                        )
                        # 发送取消结束消息
                    ...
                    if time.time() - start_time > TIMEOUT_SECONDS:
                        logger.error(
                            f"Agent execution timeout after {TIMEOUT_SECONDS}s for run_id: {ctx.run_id}"
                        )
                ...
                if cancelled.is_set():
                    logger.info(
                        f"Producer exception after cancel for run_id: {ctx.run_id}, ignoring: {ex}"
                    )
                    return
                # 使用错误分类器获取错误码
        ...
        try:
            ...
            logger.info(
                f"Stream cancelled for run_id: {ctx.run_id}, signaling producer to stop"
            )
...
async def http_run(request: Request) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            logger.error(
                f"Run execution timeout after {TIMEOUT_SECONDS}s for run_id: {run_id}"
            )
            task.cancel()
            try:
                result = await task
        ...
        logger.error(
            f"JSON decode error in http_run: {e}, traceback: {traceback.format_exc()}"
        )
        raise HTTPException(
            status_code=400, detail=f"Invalid JSON format, {extract_core_stack()}"
        )
...
async def http_stream_run(request: Request):
    ...
    try:
        ...
        logger.error(
            f"JSON decode error in http_stream_run: {e}, traceback: {traceback.format_exc()}"
        )
        raise HTTPException(
            status_code=400, detail=f"Invalid JSON format:{extract_core_stack()}",
    ...
    async def cancellable_stream():
        ...
        try:
            ...
            # 使用错误分类器获取错误码
            err = service.error_classifier.classify(
                ex, {"node_name": "http_stream_run", "run_id": run_id}
            )
            logger.error(
...
async def http_node_run(node_id: str, request: Request):
    ...
    try:
        ...
        logger.error(
            f"JSON decode error in http_node_run: {e}, traceback: {traceback.format_exc()}"
        )
        raise HTTPException(
            status_code=400, detail=f"Invalid JSON format:{extract_core_stack()}"
        )
...
def parse_args():
    ...
    parser.add_argument(
        "-m", type=str, default="http", help="Run mode, support http,flow,node"
    )
    parser.add_argument("-n", type=str, default="", help="Node ID for single node run")
    parser.add_argument("-p", type=int, default=5000, help="HTTP server port")
    parser.add_argument(
        "-i", type=str, default="", help="Input JSON string for flow/node mode"
    )
...
def _to_json_serializable(obj: Any) -> Any:
    """Convert agent result (may contain AIMessage etc) to JSON-serializable form."""
    ...
    if hasattr(obj, "dict"):
        return _to_json_serializable(obj.dict())
    return str(obj)
...
if __name__ == "__main__":
    ...
    if args.m == "http":
        ...
        print(json.dumps(_to_json_serializable(result), ensure_ascii=False, indent=2))
