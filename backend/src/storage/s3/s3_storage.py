...
class S3SyncStorage:
    ...
    def _get_client(self):
        if self._client is None:
            endpoint = self.endpoint_url or os.environ.get("BUCKET_ENDPOINT_URL") or ""
            if not endpoint or not endpoint.strip():
                raise ValueError(
                    "未配置存储端点：请设置 BUCKET_ENDPOINT_URL 或传入 endpoint_url"
                )
            ...
            # Optional: inject x-storage-token for platform; locally use env STORAGE_TOKEN or skip
            def _inject_header(**kwargs):
                ...
                if not token:
                    try:
                        ...
                        wic = WorkloadIdentityClient()
                        try:
                            token = wic.get_access_token()
                        finally:
                            wic.close()
                    except Exception:
                        return
    ...
    def generate_presigned_url(self, *, key: str, bucket: Optional[str] = None, expire_time: int = 1800) -> str:
        """通过 S3 Proxy 生成签名 URL。本地运行时使用环境变量 STORAGE_TOKEN。"""
        import json
        ...
        if not token:
            try:
                ...
                raise RuntimeError(
                    "获取 x-storage-token 失败: 本地请设置环境变量 STORAGE_TOKEN"
                ) from e
        if not token:
            raise RuntimeError("STORAGE_TOKEN 未设置，无法生成签名 URL")
        try:
