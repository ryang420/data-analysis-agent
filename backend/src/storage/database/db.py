...
def get_db_url() -> str:
    """Build database URL from environment. Set PGDATABASE_URL in .env for local runs."""
    url = os.getenv("PGDATABASE_URL") or ""
    if not url or not url.strip():
        raise ValueError(
            "PGDATABASE_URL is not set. For local runs, set it in .env (e.g. postgresql://user:pass@localhost:5432/dbname)"
        )
    return url
_engine = None
_SessionLocal = None
